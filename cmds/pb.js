...
message.channel.send(`**__Word Personal Bests for ${message.author.username}__**`);
  ...
  function verifyTimeDefined(element) {
    if (maxesTime[element]) {
      scoreTimeEmbed.addField(`**${element}s**`, `${maxesTime[element]} wpm (${findTimeRaw(element) === undefined ?'-':findTimeRaw(element)} raw) ${findTimeAcc(element)}%`);
  ...
  function verifyWordDefined(element) {
    if (maxesWords[element]) {
      scoreWordsEmbed.addField(`**${element}s**`, `${maxesWords[element]} wpm (${findWordRaw(element) === undefined ?'-':findWordRaw(element)} raw) ${findWordAcc(element)}%`);
